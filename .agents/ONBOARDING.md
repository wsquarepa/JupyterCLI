...
- **`tui/mod.rs`** owns `dashboard_loop`: a `tokio::select!` over crossterm events, an mpsc channel of `AppEvent`s from background tasks, and a 100ms tick. There is no periodic auto-refresh: the servers list updates on startup, on `r`, and via silent post-operation reconciles. It drains `app.take_effects()` each iteration and routes each `Effect`.
...
- **Client-side expectations**: terminal create/kill are optimistic (`Ghost`, `Dissolve` in `app.rs`), keyed by op id and rolled back on `OpFailed`; server spawn and fetches are server-authoritative (`SpawnView`, `GridFetch`). `Refresh`/`FetchTerminals` carry `loud: bool`: user-initiated loads get spinner labels and loading visuals, silent reconciles never repaint or animate.
